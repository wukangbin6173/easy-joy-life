...
```bash
# 如果你有SSH私钥文件
ssh -i /path/to/your/private-key root@121.43.96.127
...
确保域名 `easyjoylife.xin` 已正确解析到服务器IP：

### 检查DNS解析
```bash
# 在本地电脑运行
nslookup easyjoylife.xin
ping easyjoylife.xin
```
...
curl https://easyjoylife.xin/api/stores
```

### 2. 浏览器访问测试
- **网站首页**: https://easyjoylife.xin
- **API接口**: https://easyjoylife.xin/api/stores
...
1. 进入 **开发** -> **开发设置**
2. 配置 **服务器域名**：
   - request合法域名: `easyjoylife.xin`
   - uploadFile合法域名: `easyjoylife.xin`
   - downloadFile合法域名: `easyjoylife.xin`
...
# 检查DNS解析
dig easyjoylife.xin
nslookup easyjoylife.xin 8.8.8.8
```

#### 3. SSL证书申请失败
```bash
# 手动申请证书
certbot certonly --standalone -d easyjoylife.xin -d www.easyjoylife.xin
```
